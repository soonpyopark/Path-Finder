...
const NAME_HEADERS = [VISIT_SHEET_HEADERS[0], "기관명", "학교명", "학교", "시설명", "이름", "name"];
const TYPE_HEADERS = [VISIT_SHEET_HEADERS[1], "구분", "유형", "type"];
const DISTRICT_HEADERS = [VISIT_SHEET_HEADERS[2], "시군구", "구", "지역", "district"];
const ADDRESS_HEADERS = [VISIT_SHEET_HEADERS[3], "주소", "address"];
const HEADER_ROW_NAMES = new Set<string>([...VISIT_SHEET_HEADERS, "기관명", "학교명"]);
...
export async function exportSelectedVisits(selected: Institution[]): Promise<void> {
  ...
  const rows: Array<Array<string | number>> = [
    [...VISIT_SHEET_HEADERS],
    ...selected.map((item) => [item.name, visitTypeLabel(item.type), item.district, item.address]),
  ];

  const { downloadWorkbook } = await import("@/lib/excel-file");
  await downloadWorkbook("PathFinder_방문지.xlsx", [
    { name: "방문지", rows, columnWidths: [28, 14, 16, 42] },
  ]);
}
  ...
  void import("@/lib/excel-file").then(({ downloadWorkbook }) =>
    ...
    },
    {
      ...
      rows: [
        ...
        ["2. 1행(기관(학교)명 (필수입력), 구분(선택), 시군구(선택), 주소(선택))은 제목이니 지우거나 바꾸지 마세요."],
        ["3. 2행부터 한 줄에 한 곳씩 적습니다. 샘플 3줄은 지우고 사용해도 됩니다."],
        ["4. 파일을 저장한 뒤 Path Finder의 '엑셀 가져오기'로 올리면 선택 목록에 추가됩니다."],
        [""],
        ["■ 열 작성 방법"],
        ["기관(학교)명 (필수입력) : 공식 명칭을 적습니다. 예) 대구고등학교, 대구광역시군위교육지원청, 스타벅스 대구동성로점"],
        ...
        [""],
        ["■ 이렇게 적어도 됩니다"],
        ["- 기관(학교)명만 적어도 검색합니다. 예: 대구고등학교"],
        ["- 학교는 나이스·내부 목록에서 찾고, 없으면 상호·주소로 지도 검색합니다."],
        ["- 엑셀에서 기관(학교)명 열만 복사해 '붙여넣기'로 넣어도 됩니다. 한 줄에 한 곳입니다."],
        [""],
        ["■ 주의"],
        ["- 빈 줄은 무시됩니다."],
...
function pickField(row: Record<string, unknown>, headers: string[]): string {
  ...
  for (const header of headers) {
    ...
    const found = entries.find(([key]) => key.trim().startsWith(header));
    if (found && cellText(found[1])) return cellText(found[1]);
  }
  return "";
}
...
export function parseVisitPaste(text: string): VisitRow[] {
  return text
    ...
    .filter((line) => {
      ...
      return line.length > 0 && !HEADER_ROW_NAMES.has(first);
    })
    .map((line) => {
